...
namespace spec\TrainjunkiesPackages\NetworkRailScheduleFileParser\Line;
...
class TiplocSpec extends ObjectBehavior
{
    ...
    function let(\SplFileObject $splFileObject)
    {
        ...
        $splFileObject->beConstructedWith(['root/var/network_rail/schedule']);
        $this->beConstructedWith($splFileObject);
    }

    function it_is_a_schedule_line_parser()
    {
        $this->shouldBeAnInstanceOf(ParserAbstract::class);
        $this->shouldImplement(Line::class);
    }

    function it_can_read_tiploc_line()
    {
        $this->startsWith()->shouldBe('{"TiplocV1"');
    }

    function it_can_get_line_from_schedule_file()
    {
        $this->get(function($tiploc) {
            expect($tiploc)->shouldBeAnyOf([
                self::TIPLOC_L1,
                self::TIPLOC_L2,
                self::TIPLOC_L3,
                self::TIPLOC_L4,
    ...
    {
        return [
            'beAnyOf' => function ($subject, $arguments) {
